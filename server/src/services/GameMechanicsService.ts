...
export interface Challenge {
  ...
  criteria: {
    type: 'questions' | 'topics' | 'streak' | 'login' | 'specific';
    count: number;
    specific?: string;
  };
}
...
class GameMechanicsService {
  ...
  private readonly levelThresholds: Record<string, { min: number, max: number, stars: number }> = {
   ...
   */
  async updateDailyStreak(userId: number): Promise<number> {
    ...
    await this.checkChallenges(userId);
   ...
   */
  async recordQuestion(userId: number, questionText: string, topic?: string): Promise<void> {
    const user = await User.findByPk(userId);
    if (!user) throw new Error('User not found');
   ...
   */
  async recordTopicExploration(userId: number, topic: string): Promise<void> {
    const user = await User.findByPk(userId);
    if (!user) throw new Error('User not found');
   ...
   * Get next level info
   */
  getNextLevelInfo(currentLevel: string): { name: string, xpRequired: number } | null {
    ...
    if (nextLevel && this.levelThresholds[nextLevel as keyof typeof this.levelThresholds]) {
      return {
        name: nextLevel,
        xpRequired: this.levelThresholds[nextLevel as keyof typeof this.levelThresholds].min
      };
    }
   ...
   */
  async checkChallenges(userId: number): Promise<Challenge[]> {
    const user = await User.findByPk(userId);
    if (!user) throw new Error('User not found');
  ...
  async getUserChallenges(userId: number): Promise<any[]> {
    ...
    return this.challenges.map(challenge => {
      ...
      return {
        ...challenge,
        progress,
        total: challenge.criteria.count,
  ...
  async getUserGameProfile(userId: number): Promise<any> {
    ...
    if (nextLevel) {
      const levelKey = (user.level || 'Novice') as keyof typeof this.levelThresholds;
